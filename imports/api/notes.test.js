...
if (Meteor.isServer) {
    describe('notes', function () {
        ...
        it('should throw error if extra updates provided', function() {
            expect(() => {
                Meteor.server.method_handlers['noted.update'].apply({
                    userId: notes.userId
                }, [
                    noteOne._id,
                    { nuclear_passcode: 169878 }
                ]);
            }).toThrow();
        });
        ...
        it('should return no notes for user that has none', function() {
            const res = Meteor.server.publish_handlers.notes.apply({ userId: 'nonexistant' });
            const notes = res.fetch();
